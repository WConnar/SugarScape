...
class ReplacementRule {
  private int minAge;
  private HashMap<Agent,Integer> maxAge;
  AgentFactory fac;
  ...
  */
  public ReplacementRule(int minAge, HashMap<Agent,Integer> maxAge, AgentFactory fac) {
    this.minAge = minAge;
    this.maxAge = maxAge;
    this.fac = fac;
  }
  ...
  */
  public boolean replaceThisOne(Agent a) {
    if (a.getLifespan() == Agent.NOLIFESPAN) {
      Random r = new Random();
      a.setLifespan(minAge + r.nextInt(maxAge.get(a)+1 - minAge));
    }
    else {
      if (a.getAge() > a.getLifespan()) {
        a.setAge(maxAge.get(a)+1);
        return true;
      }
    }
    return false;
